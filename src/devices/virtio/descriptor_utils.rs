...
use vm_memory::{
    Address, ByteValued, Bytes, GuestMemory,
    GuestMemoryError, GuestMemoryRegion, Le16, Le32, Le64, VolatileMemory, VolatileMemoryError, VolatileSlice,
};

use crate::devices::file_traits::{FileReadWriteAtVolatile, FileReadWriteVolatile};
...
impl<'a> Reader<'a> {
    ...
    pub fn new(mem: &'a MemoryManager, chain: DescriptorChain<'a>) -> Result<Reader<'a>> {
...
impl<'a> Writer<'a> {
    ...
    pub fn new(mem: &'a MemoryManager, chain: DescriptorChain<'a>) -> Result<Writer<'a>> {
        let mut total_len: usize = 0;
        let buffers = chain
            .into_iter()
            .writable()
