...
use crate::devices::virtio::block::{Error, ImageType, NUM_QUEUES, QUEUE_CONFIG, SECTOR_SHIFT, SECTOR_SIZE, SyncMode};
...
use crate::devices::virtio::{ActivateError, ActivateResult, TYPE_BLOCK};
...
impl DiskProperties {
    ...
    fn build_device_id(disk_path: &Path) -> result::Result<String, Error> {
        let id = file_id::get_high_res_file_id(disk_path).map_err(Error::GetFileMetadata)?;

        let device_id = match id {
            FileId::LowRes { volume_serial_number, file_index } => format!("{:x}{:x}", volume_serial_number, file_index),
            FileId::HighRes { volume_serial_number, file_id } => format!("{:x}{:x}", volume_serial_number, file_id),
            _ => return Err(Error::GetFileMetadata(io::Error::new(io::ErrorKind::Other, "Failed to get file id"))),
        };
        Ok(device_id)
    }

    fn build_disk_image_id(disk_image_path: &str) -> Vec<u8> {
        let mut default_id = vec![0; VIRTIO_BLK_ID_BYTES as usize];
        match Self::build_device_id(Path::new(disk_image_path)) {
            Err(_) => {
...
pub struct Block {
    ...
    worker_stopfd: Arc<WindowsEvent>,
...
impl Block {
    ...
    pub fn new(
        ...
        let disk_image_id = DiskProperties::build_disk_image_id(&disk_image_path);
        ...
        Ok(Block {
            ...
            worker_thread: None,
            worker_stopfd: Arc::new(WindowsEvent::new()?),
        })
    }
...
impl VirtioDevice for Block {
    ...
    fn reset(&mut self) -> bool {
        if let Some(worker) = self.worker_thread.take() {
            let _ = self.worker_stopfd.signal();
